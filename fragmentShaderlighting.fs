...
vec3 CalcPointLight(Material material, PointLight light, vec3 N, vec3 fragPos, vec3 V)
{
    ...
    vec3 ambient = vec3(material.ambient) * light.ambient;
    vec3 diffuse = vec3(material.diffuse) * max(dot(N, L), 0.0) * light.diffuse;
    vec3 specular = vec3(material.specular) * pow(max(dot(V, R), 0.0), material.shininess) * light.specular;

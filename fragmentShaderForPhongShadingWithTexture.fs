...
vec3 CalcPointLight(Material material, PointLight light, vec3 N, vec3 fragPos, vec3 V)
{
    ...
    vec3 diffuse = vec3(texture(material.diffuse, TexCoords).rgb) * max(dot(N, L), 0.0) * light.diffuse;
    vec3 specular = vec3(texture(material.specular, TexCoords).rgb) * pow(max(dot(V, R), 0.0), material.shininess) * light.specular;
